...
class InsertOrUpdate extends Sql {
    ...
    toString() {
        if (this._where.isEmpty())
            return `${new InsertSql(this._table, this._data)} on duplicate key update ${new Raw().wrapSet(this._data)}';`;
    ...
    args() {
        if (this._where.isEmpty())
            return new InsertSql(this._table, this._data).push(new Raw().wrapSet(this._data).args()).args();
...
class Engine {
    ...
    SingleSQL(sql, args) {
        return new Promise((resolve, reject) => {
            this.getConn().then(function(conn) {
    ...
    insertOrUpdate(table, data) {
        return new InsertOrUpdate(table, data).engine(this);
    }
    ...
    where(key, op, value) {
        if (op != undefined && value != undefined) {
            ...
            return new WhereBuilder(`${key}=?`, [utils.val(op)]);
        }
